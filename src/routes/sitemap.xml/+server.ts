...
export const GET: RequestHandler = async ({ url }) => {
	...
	const staticPaths = [
		'/',
		...(site.publicDiscoveryEnabled ? ['/discover'] : []),
	...
	const entries = [
		...staticPaths.map((path) => ({ loc: new URL(path, url.origin).href, lastmod: staticLastModified })),
		...publicProjects.map((project) => ({
			loc: new URL(`/p/${project.slug}`, url.origin).href,
			lastmod: new Date(project.updatedAt * 1000).toISOString().slice(0, 10)
		}))
	];
	const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${entries.map((entry) => `  <url><loc>${escapeXml(entry.loc)}</loc><lastmod>${entry.lastmod}</lastmod></url>`).join('\n')}\n</urlset>`;
